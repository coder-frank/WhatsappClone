...
class MessageController extends Controller
{
    public function getMessage($friendId)
    {
        ...
        $messages = $messages->map(function ($msg) {
            ...
            $preview = match ($type) {
                ...
                'audio' => '<div class="custom-audio-player d-flex align-items-center gap-3 px-3 py-2 rounded-pill shadow-sm" style="max-width: 280px;">
                    <button class="btn btn-sm btn-light play-audio rounded-circle d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                        <i class="fas fa-play"></i>
                    </button>
                    <div class="flex-grow-1">
                        <div class="progress bg-secondary" style="height: 4px; border-radius: 2px;">
                            <div class="progress-bar bg-success" role="progressbar" style="width: 0%" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>
                    </div>
                    <span class="duration small text-light">0:00</span>
    ...
    public function sendMedia(Request $request)
    {
        ...
        $type = match (true) {
            ...
            str_contains($mime, 'audio') || in_array($ext, ['mp3', 'wav', 'webm']) => 'audio',
